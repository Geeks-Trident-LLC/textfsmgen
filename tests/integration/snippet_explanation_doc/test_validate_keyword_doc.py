...
Run pytest in the project root to execute these tests:
    $ pytest tests/integration/snippet_explanation_doc/test_validate_keyword_doc.py
    or
    $ python -m pytest tests/integration/snippet_explanation_doc/test_validate_keyword_doc.py
"""
...
def test_prefix_zero_or_one(category, or_empty, expected):
    for key in get_keywords(category=category):
        ...
        failure = (
            f"(zero_or_one_{key}|{category}|{or_empty}|{expected}) => {keyword_doc}"
        )
        assert expected in keyword_doc, failure
...
def test_prefix_zero_or_more(category, or_empty, expected):
    for key in get_keywords(category=category):
        ...
        failure = (
            f"(zero_or_more_{key}|{category}|{or_empty}|{expected}) => {keyword_doc}"
        )
        assert expected in keyword_doc, failure
...
def test_prefix_one_or_more(category, or_empty, expected):
    for key in get_keywords(category=category):
        ...
        failure = (
            f"(one_or_more_{key}|{category}|{or_empty}|{expected}) => {keyword_doc}"
        )
        assert expected in keyword_doc, failure
