...
@pytest.mark.parametrize(
    ...
    [   # name                  # pattern               # expected
        ...
        # --- Number ---
        ("3_number",            rf"{number}({sep}{number}){{2}}",               rf"({number}({sep}{number}){{2}})?"),
        ("3_mixed_number",      rf"{mixed_number}({sep}{mixed_number}){{2}}",   rf"({mixed_number}({sep}{mixed_number}){{2}})?"),
        ...
        ("3_letter_or_punct",   rf"{letter_or_punct}{{3}}",     rf"({letter_or_punct}{{3}})?"),
        ...
        # --- mixed-word ---
        ("3_mixed_word",        rf"{mixed_word}({sep}{mixed_word}){{2}}",   rf"({mixed_word}({sep}{mixed_word}){{2}})?"),
        ("3_mixed_words",       rf"{mixed_word}({sep}{mixed_word}){{2}}",   rf"({mixed_word}({sep}{mixed_word}){{2}})?"),

        # --- non-whitespace(s) ---
        ("3_non_ws",            rf"{non_ws}{{3}}",      rf"({non_ws}{{3}})?"),
        ("3_non_wss",           rf"{non_ws}{{3}}",      rf"({non_ws}{{3}})?"),

        ("3_non_ws_group",      rf"{non_wss}({sep}{non_wss}){{2}}",     rf"({non_wss}({sep}{non_wss}){{2}})?"),
        ("3_non_wss_group",     rf"{non_wss}({sep}{non_wss}){{2}}",     rf"({non_wss}({sep}{non_wss}){{2}})?"),
